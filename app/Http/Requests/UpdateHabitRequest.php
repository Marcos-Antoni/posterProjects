...
{
    /**
     * Only the habit's owner may update it.
     */
    public function authorize(): bool
    {
        ...
        return $this->user()?->can('update', $habit) ?? false;
    }

    /**
     * The edit dialog always submits the full form, so the rules mirror
     * `StoreHabitRequest` (including the `exclude_unless` behavior) —
     * the controller resets the conditional fields to null before
     * applying the validated payload, so switching type or recurrence
