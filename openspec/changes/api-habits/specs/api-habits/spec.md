# API Habits Specification — Delta (`api-habits`)
...
list element, and the UTC-6 23:30/23:31 boundary test binding `recordEntry()` and
`decrementToday()`'s day anchors to the same row).*
