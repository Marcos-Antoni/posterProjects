...
The stored hash MUST be a plain, unsalted `sha256` digest, never a salted hash (`bcrypt`/`argon2`/
`Hash::make`). Redemption (Requirement: Atomic Single-Use Consumption) must locate the row by an
equality lookup on the presented pass; a salted hash cannot be looked up by equality — it can only be
verified against one already-fetched row — which would force exactly the read-then-write shape the
atomic-consumption requirement forbids.
...
This is a hard requirement, not an optimization: a read-then-check-then-write shape (`SELECT` the
row, check its state in application code, then `UPDATE`) reopens the exact race two phones scanning
the same frame would exploit, because both readers could observe the pass as still-live before
either writes. The single conditional `UPDATE` pushes the exclusivity check into the database engine
itself, so `affected === 1` is a row-lock result, not application logic that can be raced.
...
This gate exists so the theft-detection signal (Requirement: Web Feedback On Consumption) can never
be destroyed silently: an automatic re-mint that quietly replaced a consumed row would erase the
owner's only evidence that someone else redeemed their pass. Requiring an explicit, owner-initiated
acknowledgement keeps that evidence on screen until the owner has actually seen it.
