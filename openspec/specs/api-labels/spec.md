...
A paginated `/comments` endpoint was explicitly declined during this change (see the
`api-labels-comments` archive record): comments are already embedded read-only via the `api-issues`
detail response, so a dedicated endpoint was judged unnecessary duplication.
...
rule. Each element MUST carry exactly four fields: `id`, `name`, `issues_count`, `updated_at`
(ISO-8601, nullable via `?->`).
...
mirroring the existing convention set by the `api-board-sprints` spec. No response body MUST
contain an `App\Models\Project` substring.
